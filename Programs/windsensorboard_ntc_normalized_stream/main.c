...
void help() {
    ...
    printf("Usage: windsensorboard_ntc_normalized_stream [OPTIONS]\n");
...
void algorithm_run() {
    ...
    // Start the stream
    i2c_stream_read_start(&windsensorboard.ntc_normalized.stream);

    // Read from the stream
    while (1) {
        // Read the next samples from the stream
        i2c_stream_read_next(&windsensorboard.ntc_normalized.stream);

        // Print all (non-empty) blocks
        for (i = 0; i < windsensorboard.ntc_normalized.stream.result.blocks_count; i++) {
            int block_status = i2c_stream_readdata_blockstatus(&windsensorboard.ntc_normalized.stream, i);
            if (block_status == I2C_STREAM_BLOCKSTATUS_BUFFER_OVERRUN) {
                ...
                if (algorithm.verbosity > 0) {
                    printf("$NTC_NORMALIZED,%d,%d,%d,%d,%d,%d,%d,%d\n", block_number,
                            i2c_stream_readdata_uint16(&windsensorboard.ntc_normalized.stream, i, 2),
                            i2c_stream_readdata_uint16(&windsensorboard.ntc_normalized.stream, i, 4),
                            i2c_stream_readdata_uint16(&windsensorboard.ntc_normalized.stream, i, 6),
                            i2c_stream_readdata_uint16(&windsensorboard.ntc_normalized.stream, i, 8),
                            i2c_stream_readdata_uint16(&windsensorboard.ntc_normalized.stream, i, 10),
                            i2c_stream_readdata_uint16(&windsensorboard.ntc_normalized.stream, i, 12),
                            i2c_stream_readdata_uint16(&windsensorboard.ntc_normalized.stream, i, 14)
                            );
                }
                block_number++;
            }
