...
static void motor_init()
{
  ...
  /* Initialize encoders */
  khepera3_drive_reset_position(0,0);
}
...
static void ir_calibrate()
{
  ...
  for (i = 0; i < 8; i++) {
    ...
    for (j = 0; j < 9; j++) {
      ir_mean[j] += khepera3.infrared_proximity.sensor[j];
    }
  }
  for (i=0; i < 9; i++) {
...
static int braitenberg(int *speed_left, int *speed_right)
{
  ...
  static const int weights[2][9] = { {0,70,100,100,-100,-100,-70,0,0},   // left wheel
                                     {0,-70,-100,-100,100,100,70,0,0} }; // right wheel
  ...
  khepera3_infrared_proximity();
  for (i = 0; i < 9; i++) {
    ir[i] = khepera3.infrared_proximity.sensor[i] - ir_zero[i];
  }
