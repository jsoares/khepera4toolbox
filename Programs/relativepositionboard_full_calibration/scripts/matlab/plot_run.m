...
%    'positions.txt' file and the one or several 'rssi_robot_x.txt' files.
    ...
    duration = 0.5;                      % frame length
    speed_up = 20;                       % speed up
    radius   = 0.06;                     % robots radius
    colors   = {'b-', 'g-', 'r-', 'k-'}; % robots colors
    ...
    for i = 0:duration:t(end)
        ...
        for j = 1:nrobots
            ...
            for k = 1:nrobots
                if (k == j), continue; end
                rssi = interp1(rssis{j,k}.t, rssis{j,k}.values, i, 'nearest');
                index = rssi(1);
                rssi = rssi(2:end);
                if (isnan(index)), continue; end
                if (abs(rssis{j,k}.t(index) - i) > 0.5), continue; end
    ...
    function draw_robot(x_, y_, a_, id_)
        ...
        plot(p_(1,:), p_(2,:), colors{mod(id_-1, length(colors))+1});
    ...
    function draw_rssi(rssi_, x_, y_, a_, id2_)
        for r_ = 1:8
            ...
            fill([dir_x1_ dir_x0_(2) fliplr(dir_x2_) dir_x0_(1)], [dir_y1_ dir_y0_(2) fliplr(dir_y2_) dir_y0_(1)], colors{mod(id2_-1, 4)+1}, 'FaceAlpha', 0.5);
        end
    end
end